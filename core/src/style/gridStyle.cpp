...
void GridStyle::constructShaderProgram() {
    m_shaderProgram->setSourceStrings(SHADER_SOURCE(grid_fs),
                                      SHADER_SOURCE(grid_vs));
}

struct GridStyleBuilder : public StyleBuilder {
    GridStyleBuilder(const GridStyle& style) : StyleBuilder(style), m_style(style) {}
